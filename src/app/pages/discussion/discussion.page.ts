...
export class DiscussionPage implements OnInit {
  ...
  discussionMessageClass = "recvdMessage"
  username = "nischaya"
  consultant = "mamta"
  queryId:any;
  ...
  getDiscussions() {
    this.apiService.getDiscussion(this.queryId)
      .subscribe(data => {
        if (data['errorCode'] == 0) {
          var discussion = JSON.parse(data['data'])
          console.log(discussion);
          this.messages = discussion
          this.isMyReplyTurn = this.messages[0].isMyReplyTurn
          this.isClosed = this.messages[0].isClosed
        }
      })
  }
  ...
  async send() {
    ...
    formData.append('isMeetingSet', 'false')


    this.apiService.sendDiscussion(formData)
      .subscribe(response => {
  ...
  formatDate(date, format: string = 'DD,MMM YYYY' ) {
    return date=='Date'? date : moment(date).format(format)
  }
  ...
  async download(url) {
    ...
    if (this.device.platform == "Android") {
      filePath = this.file.dataDirectory + fileName[fileName.length - 1]
    } else {
      filePath = this.file.documentsDirectory + fileName[fileName.length - 1]
    }
    const mimeType = MIME_TYPES['.' + filePath.split('.')[filePath.split('.').length - 1]]

    console.log("File Path ==> ", filePath);
    console.log("Mime Type ==> ", mimeType);

    this.nativeHTTP.setServerTrustMode("nocheck");
    this.nativeHTTP.downloadFile(url, {}, {}, filePath).then(async response => {
      console.log('success block...', JSON.stringify(response));
      this.fileOpner.showOpenWithDialog(filePath,mimeType)
    }).catch(err => {
  ...
  getFiles(): FileLikeObject[] {
    return this.uploader.queue.map((fileItem) => {
      // this.toUploadFiles.push(fileItem.file)
